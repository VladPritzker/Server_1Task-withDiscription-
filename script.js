window.addEventListener('DOMContentLoaded', function () {
    ...
    //         const statusMessage = document.createElement('div');// мы создали div
    ...
    //         // request.setRequestHeader('Content-type', 'multipart/form-data'); // тип данных для post  ((((((при связке XMLHttpRequest + formDAta нам заголовок устанавливать не нужно (удалить setReqeustHeader) потому что данные не попадут на сервер ))))))!!!!!!!!!!!,%,.:%,:.,:%:,%№:%№%№"%№:":%№")))
    //         const fromData = new FormData(form);// мы передаем те данные которые пользователь введет в inpute (обычная форма не JSON) и передадим их на сервер (в аргумент form)

    //         request.send(fromData); // Мы отправляем запрос с той формой которая была зполнина 
    ...
    //             }else{
    //                 statusMessage.textContent = message.failure; // при негативном результате мы вводим в поле текст с message.failure
    //             }
    //         });
    //     });
    // }
